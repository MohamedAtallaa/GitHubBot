...
class FollowerListVC: GFDataLoadingVC {
    ...
    var dataSource: DataSource<Section, Follower>!
    ...
    func updateData(on followers: [Follower]) {
        var snapshot = Snapshot<Section, Follower>()
    ...
    @objc func addButtonTapped() {
        NetworkManager.shared.getUser(for: username) { [weak self] result in
            ...
            case .success(let user):
                let favorit = Follower(login: user.login, avatarUrl: user.avatarUrl)
                PersistenceManager.updataWith(favorite: favorit, actionType: .add) { error in
